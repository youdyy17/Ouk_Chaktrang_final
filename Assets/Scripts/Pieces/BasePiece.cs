...
public abstract class BasePiece : EventTrigger
{
    ...
    // Public wrappers for manager access
    ...
    // Public accessors for current cell to support manager logic
    ...
    protected virtual void Move()
    {
        ...
        bool isCapture = mTargetCell.mCurrentPiece != null;
        ...
        // Store capture info for sound manager (sound will be played after checking for check)
        if (GameSoundManager.Instance != null)
        {
            GameSoundManager.Instance.SetPendingMoveSound(isCapture);
        }
    }
    #endregion
    ...
    public override void OnBeginDrag(PointerEventData eventData)
    {
        ...
        // Filter to only legal moves (do not allow moves that leave own king in check)
        if (mHighlightedCells.Count > 0)
        {
            List<Cell> legal = new List<Cell>();
            foreach (var cell in mHighlightedCells)
            {
                if (mPieceManager.IsLegalMove(this, cell))
                    legal.Add(cell);
            }
            mHighlightedCells = legal;
        }
